...
# ── 0. paths ────────────────────────────────────────────────────────────────
vtn_raw_dir       <- here("data", "input", "landvalues", "vtn")   # 2015-2018
vtn_preclean_dir  <- here("data", "preclean")   # 2019-2022
vtn_clean_dir     <- here("data", "clean", "vtn")
vtn_review_dir    <- here("data", "clean", "analysis_vtn")
muni_rdata        <- here("data", "clean", "muni_division_2015.Rdata")
...
clean_one_vtn <- function(path) {
  ...
  if (n_miss) {
    ...
    ensure_dir(vtn_review_dir)
    saveRDS(
      vtn |> filter(is.na(muni_code)) |> select(state, mun_name),
      file = file.path(vtn_review_dir, paste0("unmatched_", year, ".rds"))
    )
  }
