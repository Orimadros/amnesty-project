...
| 4 | **Assessing the Response from Land-Grabbers in Control Areas** (Table 3, SUTVA) | **FOUND 2026-08-06** | `fines_robustness_sutva.do` + `Multas-AvisosMatchingV1-3.R` recovered from Pedro's Dropbox (`legacy_repo/fines_robustness/`); companion data (`enforcement_clouds.dta`, DETER, `autos_infracao_df.rds`) located in the share | Download the data companions; port the matching + areg spec |
