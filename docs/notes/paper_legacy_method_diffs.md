...
# FOUND (2026-08-01): `code/working/treatmentGroups_generate.R` — the DiD vintage's source

An exhaustive sweep of `legacy_repo` (64 R files; every one checked for table- and
regression-producing code) turned up the script that produced the `amazon_working/`
intermediates the DiD blocks read. It had never been examined — it is ISO-8859 encoded
with very long lines, so plain `grep` skipped it as binary until forced with `-a`.

**The legacy repo's own DAG classifies it `deprecated`** (`build/dag/classified_files.txt`
line 994) while `2_empirics.R` is `active`. So the DiD panels were built by a
deprecated script and never regenerated — D-C is now confirmed from the source side,
not inferred.

## How its rules differ from `2_empirics.R`

| | 2_empirics.R (Table 1) | treatmentGroups_generate.R (DiD vintage) |
|---|---|---|
| target layer | CNFP **SHP_2013**, `protecao == SEM DESTINACAO` | CNFP **2020** + i3geomap `glebas_federais` |
| control layer | CNFP federal `classe` UC\|TI | separate `indigenous` + `conservation` shapefiles, after excluding state glebas |
| eligible test | occupied 2004 **AND** `dfrst__ <= 1500` **AND** `st_area <= 1500` | occupied 2004 **AND** `dfrst__ <= 1500` only — **no claim-area cap** (:279) |
| ineligible | `car2014_all` (cleaned, active) minus eligible | `ccar_dirty` minus eligible — **all target CARs, no occupancy filter, no cleaning** (:299) |
| conflict cleaning | full 5-rule algorithm | **none at all** |

That last row also explains why the paper can claim (§2.3) results carry "without any
adjustment": the DiD sample never had the adjustment.

## Why the counts still cannot be matched

Under this vintage's rules our numbers are target 164,223, eligible 107,312,
ineligible 56,911, control 13,025 -- eligible+control = 120,337 against Table 2's
implied 78,217. The gap is expected: the target layer itself differs (CNFP 2020 +
i3geomap vs SHP_2013), so the pool is a different object. We hold SHP_2013 only.

## What is NOT in the repo

- No `stargazer` / `xtable` / `kable` / `texreg` call anywhere — **Table 1 and Table 2
  were never generated by committed code**; the numbers were produced ad hoc.
- No `.do` or `.dta` files — the regressions ran in Stata on exports the repo does not
  contain, so the exact specification remains unverifiable.
- `length(unique(...))` appears once (:1341), for the eligible-side comparison only.
