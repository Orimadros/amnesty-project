...
for (path in vtn_files) {
  ...
  # diagnostics ---------------------------------------------------------------
  n_regions   <- vtn_df$region_name |> unique() |> length()
  n_unmatched <- vtn_df |> filter(is.na(region_name)) |> nrow()
  ...
  df <- vtn_df |>
    ...
    summarise(
      across(
        starts_with("receita"),                 # ← back to original rule
        ~ mean(.x, na.rm = TRUE),
        .names = "{.col}"
      ),
      .groups = "drop"
  ...
  # ── 5. Write CSV -----------------------------------------------------------
  out_path <- glue("data/clean/vtn_region_{yr}.csv")
  write_csv(df, out_path)
  message("    ↳ wrote ", out_path, "\n")
}

message("✅  Region-level CSVs saved in data/clean/")
