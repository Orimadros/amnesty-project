###############################################################################
#  Merge CAR parcels with cleaned **VNP** (NB) price data  –– 2001-2015 panel
#  --------------------------------------------------------------------------
#  • Keeps ALL parcels (left-join on normalised region *name*)
#  • Adds every land-type price column for each target year
#  • DEBUG: prints duplicate join_keys **only for VNP data**
#  • Output folder matches the 2016-2022 script:
#      data/output/vnp_parcels_region/<year>/
#  • join_key is NOT included in any written CSVs
###############################################################################
...
car_layers <- imap(car_raw, function(layer, nm) {
  layer %>% 
    ...
    st_join(region_poly, left = FALSE) %>%             # adds region_id / state / region_name
    st_drop_geometry() %>% 
...
# ── 3. VNP wide table (2001-2015) ────────────────────────────────────────────
vnp_wide <- read_rds("data/clean/vnp/city_region_yearly_pt_pre2015.rds") %>% 
  janitor::clean_names() %>% 
  mutate(join_key = normalize_name(region_name))
...
if (nrow(vnp_dups) > 0) {
  warning(
    nrow(vnp_dups), " duplicated join_keys found in VNP pre-2015 data. ",
    "Only the first entry will be used in joins."
  )
  cat("\n── Duplicate keys in VNP pre-2015 data ──────────────────────────────\n")
  vnp_dups %>% arrange(desc(n)) %>% as.data.frame() %>% print(row.names = FALSE)
}

# Keep first occurrence of each duplicated key
vnp_wide <- vnp_wide %>% group_by(join_key) %>% slice(1) %>% ungroup()

# ── 4. Output root & year vector ─────────────────────────────────────────────
out_root <- "data/output/vnp_parcels_region"   # same root as 2016-2022
...
cat("\n📂  Parcel-level VNP CSVs (2001-2015) saved under:\n   ",
