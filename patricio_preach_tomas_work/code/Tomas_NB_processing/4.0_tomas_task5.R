###############################################################################
#  Compare CAR regions to VNP price table (2001-2015) by *name* only
#  – output shows just `region_name` plus that year’s price columns
###############################################################################
...
# ── 2. VNP wide table (pre-2015) --------------------------------------------
vnp_wide <- read_rds("data/clean/vnp/city_region_yearly_pt_pre2015.rds") |>
  janitor::clean_names() |>
  mutate(join_key = normalize_name(region_name))

# ── 3. Output folders --------------------------------------------------------
out_root <- "data/output/vnp_pre2015_vs_car_regions"
...
summary_tbl <- map_dfr(years, function(yr) {
  ...
  vnp_year <- vnp_wide |>
    select(join_key, all_of(cols_year))
