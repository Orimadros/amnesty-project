...
Project briefing for Claude Code. Read this before doing anything in the repo.
...
make docker-run    Non-TTY entry point for Claude Code (and any other agent driver). Use CMD="..." to pass a command.
...
**For Claude Code** — `make docker-boot` requires a real TTY and will fail in Claude's executor. Always use `docker-run` instead:

```bash
make docker-run CMD="Rscript r-src/some_script.R"
```

Do not use `make docker-boot` or bare `docker run -it` — they will error with "stdin is not a terminal". Do not invent shortcuts that bypass the container.
